...
- 기본타입
  - int, double같은 기본타입은 절대 공유하지 않는다.
  - 기본 타입은 항상 값을 복사한다.
  - Integer, String같은 래퍼클래스는 공유 가능한 객체이지만 변경할 수 없다.

> 임베디드 타입
- 새로운 값 타입을 직접 정의
- JPA는 embedded type 이라고 한다.
...
> 임베디드 타입과 연관관계
- JPA 표준 스펙에 나온 내용
- Entity는 vlaue를 갖는데, value는 또 다른 entity를 가질 수 있다.
  - value가 엔티티의 foreign key가지고 있으면 되기 때문

> @AttributeOverride 
- 엔티티에 임베디드 타입 2개 사용할 때
  - 예) 주소라는 임베디드 타입을 집주소, 회사 주소에서 사용할 때 

### 값타입과 불변 객체

- 깊이 있는 내용
- 값타입은 복잡한 객체 세상을 단순하게 만들기 위해 생긴 개념

> 부작용
- 임베디드 타입을 여러 엔티티에서 공유하면 위험하다.
- 객체 타입은 참조 값을 직접 대입하는 것을 막을 수 없으므로, 공유 참조는 피할 수가 없다.

> 불변객체
- 수정할 수 없게 만들면 부작용 원천 차단
- 생성 시점 이후 절대 값을 변경할 수 없는 객체
- setter를 생성하지 않고, 생성자로만 값을 설정
- Integer, String 대표적 불변 객체

> 값을 바꾸고 싶을 때는 어떻게?
- 객체를 새로 만들어 set해준다
  - 예) member.setHomeAddress(new Address(~));
